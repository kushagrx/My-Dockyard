...
- ✅ Create an SQL initialization script (e.g., `Kushagra.sql`) with database and table definitions.
...
COPY Kushagra.sql /docker-entrypoint-initdb.d/
...
Create a file named `Kushagra.sql` in the same directory:

```sql
CREATE DATABASE Kushagra;
USE Kushagra;
...
🔄 Switch to the `Kushagra` database:
